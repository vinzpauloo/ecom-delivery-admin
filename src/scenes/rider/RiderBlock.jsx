...
const RiderBlock = () => {
  const [riderBlockList, setRiderBlockList] = useState([]);
  const { getRiderDeclined } = useRiderByStatus();

  const loadRiderDeclined = async () => {
    const response = await getRiderDeclined({
      with: "user",
      approval: "Declined",
    });
    ...
    <Box p="20px" bgcolor="#454d55">
      ...
      <Box p="20px" bgcolor="#151C26">
        ...
        <Typography
          ...
          mb="10px"
        >
          BLOCK LISTED RIDERS | APPLICANTS
        </Typography>
        <Box bgcolor="#151c26" maxHeight="655px" overflow="auto">
          {/* the data is need in a array form */}
          <RiderInfo data={riderBlockList} colorTheme={colorTheme} />
        </Box>
      </Box>
    </Box>
  );
